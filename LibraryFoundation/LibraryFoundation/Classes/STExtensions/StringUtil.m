...
+(bool) isMobileNumber:(NSString *) mobile
{
    ...
    {
        //        NSString *firstStr = [mobile substringToIndexForeasy:1];
...
+(NSString *) getVerify:(NSString *)eighteen
{
    ...
    {
        eighteen = [eighteen substringToIndexForeasy:17];//eighteen.substring(0, 17);
    }
    ...
    if (eighteen.length == 17)
    {
        ...
        for (unsigned int i = 0; i < 17; i++)
        {
            ...
            NSString *k = [eighteen substringWithRangeForeasy:NSMakeRange(i, 1)];
...
+ (bool)isDateRight:(NSString *)date
{
    if(date != nil && date.length == 8)
    {
        ...
        year = [[date substringToIndexForeasy:4] intValue];
        month = [[date substringWithRangeForeasy:NSMakeRange(4, 2)] intValue];
        day = [[date substringWithRangeForeasy:NSMakeRange(6, 2)] intValue];
...
+ (NSArray *)stingToArray:(NSString *)string
{
    ...
    for (NSInteger i = 0; i < string.length; i++) {
        NSString *s  = [string substringWithRangeForeasy:NSMakeRange(i, 1)];
        [array addObject:s];
    }
