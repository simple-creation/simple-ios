...
+ (void)closeWithJsonStr:(NSString *)jsonStr callBackId:(NSString *)callBackId callback:(NativeHybirdCallBackBlcok)callback
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([UIViewController topMostViewController].navigationController) {
            [[UIViewController topMostViewController].navigationController popViewControllerAnimated:true];
        } else {
            [[UIViewController topMostViewController] dismissViewControllerAnimated:YES completion:nil];
        }
    });
}
...
+ (void)openWithJsonStr:(NSString *)jsonStr callBackId:(NSString *)callBackId callback:(NativeHybirdCallBackBlcok)callback {
    ...
    if ([dic.allKeys containsObject:@"url"]) {
        ...
        if (arry && arry.count > 0) {
            ...
            NSDictionary *paramDic = [STSchemalRule queryParameters:[arry lastObject]];
            dispatch_async(dispatch_get_main_queue(), ^{
...
//                    [currentVc.navigationController pushViewController:rnVc animated:true];
                    [STBus callData:@"rn" param:[NSURL URLWithString:url],nil];
                    
                    
                } else if (type == STSchemalH5) {
                    Class cls = NSClassFromString(@"STWKWebViewController");
                    UIViewController *new = [UIViewController new];
                    if ([currentVc navigationController]) {
                        [currentVc.navigationController setNavigationBarHidden:NO];

                        [currentVc.navigationController pushViewController:[cls new] animated:YES];
                    } else {
                        [currentVc presentViewController:new animated:YES completion:nil];
                    }
                    
                } else if (type == STSchemalFlutter) {
                    //[NSURL URLWithString:url]
                    [STBus callData:@"flutter" param:[NSURL URLWithString:url],nil];
                    
                } else {//Native
                    Class cls = NSClassFromString(@"STViewController");
                    if ([currentVc navigationController]) {
                        [currentVc.navigationController setNavigationBarHidden:NO];

                        [currentVc.navigationController pushViewController:[cls new] animated:YES];
                    } else {
                        [currentVc presentViewController:[cls new] animated:YES completion:nil];
                    }
